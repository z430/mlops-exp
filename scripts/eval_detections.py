...
def main(args):
    ...
    for sample in dataset:
        ...
        try:
            ...
            for _, row in predictions.iterrows():
                ...
                y_max = row["ymax"]
                print(
                    f"Detected {label} with confidence {confidence} at "
                    f"({x_min}, {y_min}) to ({x_max}, {y_max})"
                )

            logger.info(f"Detections saved for sample: {sample_id}")
        except Exception as e:
    ...
    new_model_name = (
        f"{args.model_name.lower()}_v{datetime.now().strftime('%Y%m%d%H%M%S')}.pt"
    )
    ...
    output_model.update_weights_package(
        os.path.join(os.path.dirname(model_path), new_model_name)
    )
